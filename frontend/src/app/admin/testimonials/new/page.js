...
export default function AddTestimonialPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const createdTestimonial = response.data?.data?.testimonial;

      toast.success(
        response.data?.message || "Testimonial created successfully.",
      );

      if (createdTestimonial?._id) {
        router.push(`/admin/testimonials/${createdTestimonial._id}`);
      } else {
      ...
      <section className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-muted sm:text-base">
            Add a student review, academic information and publishing
            permissions.
          </p>
        </div>
